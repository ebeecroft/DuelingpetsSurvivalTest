require "application_system_test_case"

class RegistrationsTest < ApplicationSystemTestCase
  setup do
    @registration = registrations(:one)
  end

  test "visiting the index" do
    visit registrations_url
    assert_selector "h1", text: "Registrations"
  end

  test "creating a Registration" do
    visit registrations_url
    click_on "New Registration"

    fill_in "Accounttype", with: @registration.accounttype_id
    fill_in "Birthday", with: @registration.birthday
    fill_in "Country", with: @registration.country
    fill_in "Country timezone", with: @registration.country_timezone
    fill_in "Email", with: @registration.email
    fill_in "Firstname", with: @registration.firstname
    fill_in "Lastname", with: @registration.lastname
    fill_in "Login", with: @registration.login_id
    fill_in "Message", with: @registration.message
    fill_in "Registered on", with: @registration.registered_on
    fill_in "Vname", with: @registration.vname
    click_on "Create Registration"

    assert_text "Registration was successfully created"
    click_on "Back"
  end

  test "updating a Registration" do
    visit registrations_url
    click_on "Edit", match: :first

    fill_in "Accounttype", with: @registration.accounttype_id
    fill_in "Birthday", with: @registration.birthday
    fill_in "Country", with: @registration.country
    fill_in "Country timezone", with: @registration.country_timezone
    fill_in "Email", with: @registration.email
    fill_in "Firstname", with: @registration.firstname
    fill_in "Lastname", with: @registration.lastname
    fill_in "Login", with: @registration.login_id
    fill_in "Message", with: @registration.message
    fill_in "Registered on", with: @registration.registered_on
    fill_in "Vname", with: @registration.vname
    click_on "Update Registration"

    assert_text "Registration was successfully updated"
    click_on "Back"
  end

  test "destroying a Registration" do
    visit registrations_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Registration was successfully destroyed"
  end
end
